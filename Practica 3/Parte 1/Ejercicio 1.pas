...
program p3p1ej1; //reutilizo codigo del ejercicio 4 de la practica 1
type
	empleado = record
		numero: integer;
		apellido: string[15];
...
procedure darDeBaja (var arc_logico: archivo);
var
	e: empleado;
	num: integer;
	encontre: boolean;
...
begin
	encontre:= false;
	writeln (' Ingrese el numero del empleado a dar de baja: ');
	readln (num);
	reset (arc_logico);
	while (not EOF (arc_logico) and (not encontre)) do begin
		read (arc_logico,e);
		if (e.numero = num) then begin //chequeo si el empleado ingresado es igual al empleado que esta en esa pos del archivo
			pos:= filepos(arc_logico)-1;
			encontre:= true;
		end;
	end;
	
	if (encontre) then begin
		seek (arc_logico,fileSize(arc_logico)-1); //muevo el puntero al ultimo lugar 
		read (arc_logico,e); //leo el elemento
		seek (arc_logico,fileSize(arc_logico)-1); //como el read me dejo apuntando al eof,vuelvo atras
		truncate(arc_logico); //adelanto el eof
		seek (arc_logico,pos); //vuelvo al lugar a dar de baja
		write (arc_logico,e); //escribo el registro completo
	end;
...
begin
	...
	writeln (' 8: Realizar una baja copiando el último registro del archivo en la posición de un registro a borrar ');
	writeln (' 9: SALIR Y TERMINAR EL PROGRAMA ');
	readln (opcion);
	...
	while (opcion <> 9) do begin
		...
		writeln (' 7: EXPORTAR A UN ARCHIVO DE TEXTO LLAMADO: " FALTADNIEMPLEADO.TXT " , LOS EMPLEADOS QUE NO TENGAN CARGADO EL DNI (DNI en 00)');
		writeln (' 8: Realizar una baja copiando el último registro del archivo en la posición de un registro a borrar ');
		writeln (' 9: SALIR DEL MENU Y TERMINAR PROGRAMA');
		readln (opcion);
	end;	
end;
